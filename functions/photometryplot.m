...
% Written by Stephen Zhang 7/2021
...
addOptional(p, 'window', [60, 60]); % Window size (pre, post)
addOptional(p, 'transitions', {'W', 'NR', [0, inf, 0, 60]}); % A cell containing the transition info.
                                                             % e.g., {'W', 'NR', [0, inf, 0, 60]}
                                                             % means no limit for pre-transition bouts and
                                                             % 0-60 seconds for post-transition bout lengths
...
% Scoring file columns
addOptional(p, 'onsetcolumn', 5); % Onsets
addOptional(p, 'durationcolumn', 4); % Durations
addOptional(p, 'lagcolumn', 6); % lag column (how many seconds that scoring lags behind photometry)
...
% Parse scoring
onsets = scoringdata(:,p.onsetcolumn);
durations = scoringdata(:,p.durationcolumn);
scoringlag = scoringdata(1,p.lagcolumn);
onsets = onsets + scoringlag;
labels = scoringtext(2:end,3);

% Make transitions
labelpairs = [labels(1:end-1), labels(2:end)];
durationpairs = [durations(1:end-1), durations(2:end)];
...
for iplot = 1 : nplots
    ...
    % Judging by durations
    dpre1 = durationpairs(:,1) >= p.transitions{iplot, 3}(1);
    dpre2 = durationpairs(:,1) < p.transitions{iplot, 3}(2);
    dpost1 = durationpairs(:,2) >= p.transitions{iplot, 3}(3);
    dpost2 = durationpairs(:,2) < p.transitions{iplot, 3}(4);
    
    inds = find(ipre & ipost & dpre1 & dpre2 & dpost1 & dpost2);
    
    if ~isempty(inds) % If no such sessions are found - skip
            
        % Onsets of the current transitions(in seconsd)
        consets = onsets(inds+1);
        nonsets = length(consets);

        % Durations of pre- and post-transition bouts
        dur_pre = durationpairs(inds,1);
        dur_post = durationpairs(inds,2);

        % Limit the durations to within window
        dur_pre = min(dur_pre, p.window(1));
        dur_post = min(dur_post, p.window(2));
        ...
        l_pre = round(p.window(1) * fs);
        l_post = round(p.window(2) * fs);
        ...
        for i = 1 : 2
            ...
            % Set the xaxis right
            xlabel('Time (s)')
            set(gca,'XTickLabel', num2cell([-p.window(1), 0, p.window(1)]));
            ...
            % title
            title(sprintf('Ch%i: %s (%i-%i) - %s (%i-%i)', i, p.transitions{iplot, 1},...
                p.transitions{iplot, 3}(1), p.transitions{iplot, 3}(2),...
                p.transitions{iplot, 2}, p.transitions{iplot, 3}(3), p.transitions{iplot, 3}(4)));
        end
        ...
        if p.savefig
            ...
            % output figure
            figfn = sprintf('%s-%s_%i.png', p.transitions{iplot, 1}, p.transitions{iplot, 2},...
                iplot);
